...
class HrJob(models.Model):
    ...
    approver_stage_1 = fields.Many2one(
        "res.users",
        string="1st Stage Approver",
    )
    approver_stage_2 = fields.Many2one(
        "res.users",
        string="2nd Stage Approver",
    )
    approver_final = fields.Many2one(
        "res.users",
    ...
    def action_submit_first(self):
        ...
        for record in self:
            ...
            record.message_post(
                body=_("Submitted to 1st Stage. Pending approval from %s")
                % record.approver_stage_1.name
            )
    ...
    def action_approve_second(self):
        for record in self:
            if self.env.user != record.approver_stage_1:
                raise UserError(
                    _(
                        "Only the First Stage Approver (%s) is authorized to approve this stage."
                    )
                    % record.approver_stage_1.name
                )
            ...
            record.message_post(
                body=_("1st Stage Approved. Now pending approval from %s")
                % record.approver_stage_2.name
            )
    ...
    def action_approve_final(self):
        for record in self:
            if self.env.user != record.approver_stage_2:
                raise UserError(
                    _(
                        "Only the Second Stage Approver (%s) is authorized to approve this stage."
                    )
                    % record.approver_stage_2.name
                )
            ...
            record.message_post(
                body=_("2nd Stage Approved. Waiting for Final Approval from %s")
                % record.approver_final.name
            )
    ...
    def action_set_to_open(self):
        for record in self:
            if self.env.user != record.approver_final:
                raise UserError(
                    _("Only the Final Approver (%s) is authorized to publish this job.")
                    % record.approver_final.name
                )

            # Mark final activity as done
            record.activity_feedback(["mail.mail_activity_data_todo"])
            record.write({"state": "open", "website_published": True})
            record.message_post(
                body=_("Job successfully approved and published by %s")
                % self.env.user.name
            )
    ...
    def action_refuse(self):
        for record in self:
            if self.env.user != record.approver_final:
                raise UserError(
                    _(
                        "Only the Final Approver (%s) is authorized to refuse this request."
                    )
                    % record.approver_final.name
                )
            ...
            record.message_post(
                body=_("Job request has been refused by %s") % self.env.user.name
            )
